...
public class AccountService {
     ...
     */
 //   @Transactional  // 이체 전체 과정을 하나의 트랜잭션으로 처리 // class level 에서 정의했으므로 필요없음 
  ...
  //     BigDecimal toBalance = new BigDecimal("0");
    ...
    private void validateTransferInput(Long fromAccountNumber, Long toAccountNumber, BigDecimal transferAmount) {
        ...
        // 이체 한도 검증 (예: 1억원 이하)
        if (transferAmount.compareTo(new BigDecimal("100000000")) > 0) {
            throw new IllegalArgumentException("이체 금액이 한도(1억원)를 초과했습니다.");
        }
    }
}